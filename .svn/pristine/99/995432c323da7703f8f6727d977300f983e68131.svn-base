app.controller('goodsController' ,function($scope,$controller,$location,goodsService,uploadService,itemcatService,typeTemplateService){
	...
	//显示分类的名称
	$scope.status=["保存未申请","申请中","申请通过","已驳回"];
	//上下架状态
	$scope.mark=["下架","上架"];
	$scope.itemCatList = [];
	...
	$scope.updateJ=function(marketableId){
		if($scope.selectIds.length==0){
			swal({    title: "至少选择一条数据！",    text: "2秒后自动关闭。",    timer: 2000,    showConfirmButton: false  });
		}
		else if ($scope.auditStatus.length==0) {
			swal({    title: "必须审核通过的商品才能进行此操作！",    text: "2秒后自动关闭。",    timer: 2000,    showConfirmButton: false  });
			console.log("11111111111+++++"+$scope.auditStatus.length);
		}else{
			
			goodsService.updateJ(marketableId,$scope.selectIds).success(
					function(response){
						if(response.success){
							$scope.reloadList();//刷新列表
							swal('已完成！', '数据列表已更新。');
